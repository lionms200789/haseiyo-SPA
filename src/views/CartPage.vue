    ...
    <div class="cart">
      <div class="row justify-content-center">
        <div class="col-md-10">
          ...
          <table class="table cart-table" v-if="myCarts.final_total > 0">
              ...
              <tr v-for="item in myCarts.carts" :key="item.id">
                ...
                <td width="10%" class="px-0">
                  <select
                    class="productQty"
                    v-model="item.qty"
                    @change="modifyQty(item.id, item.qty ,item.size, item.product_id)"
                  >
          ...
          <div v-if="myCarts.carts == ''" class="py-5 text-center">
            <div>
          ...
          <div class="cart-total">
            ...
            <div class="row py-4 justify-content-end">
              ...
              <div class="col-md-3 mb-2" v-if="myCarts.final_total > 1">
                <a href="#" @click.prevent="gocheckout" class="btn btn-dark rounded-0 w-100">前往結帳</a>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    gocheckout() {
      const vm = this;
      let cartlen = vm.myCarts.carts.length;
      if (cartlen > 0) {
        vm.$router.push("/customer/order_page");
      }
    },
    modifyQty(id, qty, size, product_id) {
      const vm = this;
      const itemData = {
        product_id,
        qty,
      ...
      vm.$http
        .all([
          vm.$http.delete(delCartApi),
          vm.$http.post(newCartApi, { data: itemData })
        ])
        .then(
          vm.$http.spread(function() {
            vm.getCart();
          })
        );
    }
